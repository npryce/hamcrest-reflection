...
public class HasAnnotationMatcherTest {
	...
	public void matchesAnnotatedMembers() throws Exception {
		final Method method = Annotated.class.getMethod("annotatedMethod");

		assertThat("has annotation", method, hasAnnotation(Deprecated.class));
		assertThat("does not have unused annotation", method, not(hasAnnotation(UnusedAnnotation.class)));
	}
	...
	public void matchesAnnotatedMembersWithProperties() throws Exception {
		...
		assertThat(method, hasAnnotation(AnnotationWithArgument.class, this.withValue(equalTo(1))));
	}
	...
	public Matcher<AnnotationWithArgument> withValue(final Matcher<? super Integer> valueMatcher) {
		return new TypeSafeMatcher<AnnotationWithArgument>() {
			@Override
			protected boolean matchesSafely(final AnnotationWithArgument item) {
			...
			@Override
			protected void describeMismatchSafely(final AnnotationWithArgument item, final Description mismatchDescription) {
			}
		};
	}
}
